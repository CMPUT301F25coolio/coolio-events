...
public class SignupActivity extends AppCompatActivity {
    ...
    TextView emailOptionalText; // Textview used for optional email for entrants
    TextView passwordOptionalText; // Textview used for optional password for entrants
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // small labels that show "optional for entrants"
        emailOptionalText = findViewById(R.id.emailOptionalText);
        passwordOptionalText = findViewById(R.id.passwordOptionalText);
        ...
        // Back Button On Click Listener that sends user back to welcome screen
        ...
        // Organizer Button On Click Listener that User chooses their account type as organizer
        ...
        // Entrant Button On Click Listener - User chooses their account type as entrant
     ...
     * @param type the type of account and 0 for Entrant, 1 for Organizer
     */
    private void changeAccountType(int type){
        ...
        if (type == 0) {
            ...
            // for organizers, these are required so we hide the helper text
            if (emailOptionalText != null) {
                emailOptionalText.setVisibility(View.GONE);
            }
            if (passwordOptionalText != null) {
                passwordOptionalText.setVisibility(View.GONE);
            }
            ...
            if (emailOptionalText != null) {
                emailOptionalText.setVisibility(View.VISIBLE);
            }
            if (passwordOptionalText != null) {
                passwordOptionalText.setVisibility(View.VISIBLE);
            }
        }
    }
    ...
    private void doSignup(){
        ...
        if (accountType == 0) {
            // organizer needs email and password
            ...
            attemptMakeOrganizerAccount(name, username, email, password);
        } else if (accountType == 1) {
            // entrant: password optional, email optional
            attemptMakeEntrantAccount(name, username, email);
        }
    }

    /**
     * Makes an organizer account using email/password auth
     */
    private void attemptMakeOrganizerAccount(String name, String username, String email, String password) {
                ...
                .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            usermap.put("email", user != null ? user.getEmail() : email);
    ...
    /**
     * Makes an entrant account identified by this device (anonymous auth).
     * Email and password are NOT used for login
     */
    private void attemptMakeEntrantAccount(String name, String username, String email) {
        mAuth.signInAnonymously()
                .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    @Override
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(SignupActivity.this, "Entrant account created.",
                                    Toast.LENGTH_SHORT).show();
                            FirebaseUser user = mAuth.getCurrentUser();

                            if (user == null) {
                                warnText.setText("Could not get user after signup.");
                                return;
                            }
                            ...
                            Toast.makeText(SignupActivity.this, "Entrant signup failed.",
                                    Toast.LENGTH_SHORT).show();
                            if (task.getException() != null) {
                                warnText.setText(task.getException().getMessage());
                            }
